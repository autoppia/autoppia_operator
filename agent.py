...
from llm_gateway import is_sandbox_gateway_base_url, openai_chat_completions
...
def _normalize_url_parsed(parsed) -> str:
    path = parsed.path or ""
    if not path:
...
def _normalize_demo_url(raw_url: str | None) -> str:
    ...
    try:
        ...
        if "." in normalized or ":" in normalized:
            return _normalize_url_parsed(urlsplit(f"http://{normalized}"))
        return f"http://localhost/{normalized}"
    except Exception:
        return "http://localhost/"
...
class _Candidate:
    ...
    def _click_selector_from_attrs(self) -> dict[str, Any] | None:
    ...
    def click_selector(self) -> dict[str, Any]:
        ...
        derived = self._click_selector_from_attrs()
    ...
    def _type_selector_from_attrs(self) -> dict[str, Any] | None:
    ...
    def type_selector(self) -> dict[str, Any]:
        ...
        if isinstance(self.selector, dict) and self.selector.get("type") == "attributeValueSelector":
            ...
            if attr and attr != "class":
                return self.selector
        derived = self._type_selector_from_attrs()
        if derived is not None:
            return derived
...
def _best_container_from_candidates(candidates: list):
    """Pick best (size, interactivity) container or fallback to first after sort."""
    best = None
    best_key = None
    for L, n_inter, node in candidates:
        if not (50 <= L <= 900) or n_inter <= 0 or n_inter > 12:
            continue
        key = (L, n_inter)
...
def _pick_context_container_bs4(el) -> object | None:
    ...
    """
    try:
        candidates = _collect_container_candidates(el)
        if not candidates:
            return None
        return _best_container_from_candidates(candidates)
    except Exception:
        return None


_STRUCTURAL_TAGS = {"header", "nav", "main", "form", "section", "article", "aside", "footer", "ul", "ol", "table", "div"}


def _ancestor_to_chain_label(a: Any) -> str | None:
    """Build a single label for an ancestor node for container chain; None if skip."""
    ...
    label = _norm_ws(" ".join([b for b in label_bits if b]))
...
def _container_chain_from_el(el: Any) -> list[str]:
    ...
    try:
        ...
        for a in reversed(ancestors):
            try:
                label = _ancestor_to_chain_label(a)
...
def _group_for_el(el) -> str:
    """Group by semantic container (NAV, HEADER, FORM, etc.)."""
    try:
...
def _should_skip_candidate(tag: str, attr_map: dict[str, str]) -> bool:
    if tag == "input" and (attr_map.get("type") or "").lower() == "hidden":
        return True
    if attr_map.get("disabled") is not None:
        return True
    if (attr_map.get("aria-disabled") or "").lower() == "true":
        return True
    return _is_hidden_candidate_attr(attr_map)


def _context_and_title_for_el(el) -> tuple[str, str, str]:
    """Return (context, context_raw, title) for an element's parent container."""
    try:
        ...
        title = _norm_ws(h.get_text(" ", strip=True)) if h is not None else ""
        if not title:
            t = parent.find(attrs={"class": re.compile(r"title", re.I)})
            title = _norm_ws(t.get_text(" ", strip=True)) if t is not None else ""
        return context, context_raw, title
    except Exception:
        return "", "", ""


def _select_options_from_el(el) -> list:
...
def _select_primary_and_label(el, primary: dict[str, Any], dom_label: str, opts: list) -> tuple[dict[str, Any], str]:
    ...
    if isinstance(primary, dict) and primary.get("type") == "attributeValueSelector" and str(primary.get("attribute") or "") == "custom" and str(primary.get("value") or "") == "select":
        first_opt = str(opts[0][0] or "").strip() if opts else ""
        if first_opt:
            safe = first_opt.replace('"', "'")
            primary = _sel_custom(f'select:has(option:has-text("{safe}"))')
    if opts:
        show = [f"{t} (value={v})" if v and v != t else t for t, v in opts[:8]]
        label = (dom_label or "select") + f" options=[{', '.join(show)}]"
        label = label[:200]
...
def _dom_digest_title(soup) -> str:
    try:
        if soup.title and soup.title.get_text(strip=True):
            return _norm_ws(soup.title.get_text(" ", strip=True))[:160]
    except Exception:
        pass
    return ""


def _dom_digest_headings(soup) -> list[str]:
    heads = []
...
def _dom_digest_forms_bits(soup) -> list[str]:
    bits = []
    for form in soup.find_all("form", limit=4):
        items = []
        for el in form.find_all(["input", "textarea", "select"], limit=12):
            try:
                ...
                continue
        if items:
            bits.append("; ".join(items[:8]))
    return bits[:3]


def _dom_digest_ctas(soup) -> list[str]:
    ctas = []
    for el in soup.select("button,a[href],[role='button'],[role='link']"):
        if len(ctas) >= 14:
            break
        try:
            t = _norm_ws(el.get_text(" ", strip=True)) or _norm_ws(str(el.get("aria-label") or "") or "")
            if not t or t.lower() in {"home", "logo"} or t in ctas:
                continue
            ctas.append(t[:90])
        except Exception:
            continue
    ...
    try:
        ...
        for t in soup(["script", "style", "noscript"]):
            with contextlib.suppress(Exception):
                t.decompose()
        parts = []
        title = _dom_digest_title(soup)
        if title:
            parts.append(f"TITLE: {title}")
        heads = _dom_digest_headings(soup)
        if heads:
            parts.append("HEADINGS: " + " | ".join(heads))
        forms_bits = _dom_digest_forms_bits(soup)
        if forms_bits:
            parts.append("FORMS: " + " || ".join(forms_bits))
        ctas = _dom_digest_ctas(soup)
        if ctas:
...
def _ir_title_headings_from_soup(html: str) -> tuple[str, list]:
    """Extract title and headings from HTML for page IR."""
    if BeautifulSoup is None:
        return "", []
    try:
        soup = BeautifulSoup(html, "lxml")
        for t in soup(["script", "style", "noscript"]):
            with contextlib.suppress(Exception):
                t.decompose()
        title = _norm_ws(soup.title.get_text(" ", strip=True))[:120] if soup.title else ""
        heads = []
        for h in soup.find_all(["h1", "h2", "h3"], limit=12):
            tx = _norm_ws(h.get_text(" ", strip=True))
            if tx and tx not in heads:
                heads.append(tx[:100])
        return title, heads[:10]
    except Exception:
        return "", []
...
def _ir_cleaned_links(links_obj: Any, max_links: int) -> list:
    if not isinstance(links_obj, dict) or not links_obj.get("ok"):
        return []
    links = links_obj.get("links") if isinstance(links_obj.get("links"), list) else []
    clean = []
    for link in links[:max_links]:
        if not isinstance(link, dict):
            continue
        text = _norm_ws(str(link.get("text") or ""))
        href = _norm_ws(str(link.get("href") or ""))
        ctx = _norm_ws(str(link.get("context") or ""))
        if not text and not href:
            continue
        clean.append({"text": text[:90], "href": href[:150], "ctx": ctx[:140]})
    return clean[:max_links]


def _ir_cleaned_cards(cards_obj: Any, max_cards: int) -> list:
    if not isinstance(cards_obj, dict) or not cards_obj.get("ok"):
        return []
    cards = cards_obj.get("cards") if isinstance(cards_obj.get("cards"), list) else []
    out = []
    for c in cards[:max_cards]:
        ...
        actions_clean = [
            {"tag": str(a.get("tag") or ""), "text": _norm_ws(str(a.get("text") or ""))[:90], "href": _norm_ws(str(a.get("href") or ""))[:120]} for a in actions[:3] if isinstance(a, dict)
        ]
        out.append({"facts": [str(x)[:100] for x in (c.get("card_facts") or [])[:4]], "actions": actions_clean, "text": _norm_ws(str(c.get("card_text") or ""))[:260]})
    return out


def _ir_ctas_from_candidates(candidates: list[_Candidate]) -> list[str]:
    ctas = []
    for c in candidates:
        if c.tag not in {"button", "a"}:
            continue
        tx = _norm_ws(c.text or "")
        if not tx or tx.lower() in {"home", "logo"} or tx in ctas:
            continue
        ctas.append(tx[:90])
        if len(ctas) >= 16:
            break
...
def _extract_page_ir(*, html: str, url: str, candidates: list[_Candidate], max_forms: int = 4, max_links: int = 20, max_cards: int = 10) -> dict[str, Any]:
    ...
    ir: dict[str, Any] = {"title": "", "url_path": "", "headings": [], "forms": [], "ctas": [], "links": [], "cards": []}
    if not html:
        return ir
    try:
        ir["url_path"] = str(urlsplit(str(url or "")).path or "/")
    except Exception:
        ir["url_path"] = str(url or "")
    title, heads = _ir_title_headings_from_soup(html)
    ir["title"] = title
    ir["headings"] = heads
    ir["forms"] = _ir_cleaned_forms(_tool_extract_forms(html=html, max_forms=max_forms, max_inputs=16), max_forms)
    ir["links"] = _ir_cleaned_links(_tool_list_links(html=html, base_url=str(url), max_links=max_links, context_max=150), max_links)
    ir["cards"] = _ir_cleaned_cards(_tool_list_cards(candidates=candidates, max_cards=max_cards, max_text=380, max_actions_per_card=3), max_cards)
    ir["ctas"] = _ir_ctas_from_candidates(candidates)
    return ir


def _render_ir_forms(forms: list) -> list[str]:
    lines = []
...
def _render_ir_links(links: list) -> list[str]:
    lines = []
    for link in links[:8]:
        if not isinstance(link, dict):
            continue
        lines.append(f"- text={str(link.get('text') or '')[:80]} href={str(link.get('href') or '')[:120]} ctx={str(link.get('ctx') or '')[:120]}")
    return lines


def _render_ir_cards(cards: list) -> list[str]:
    lines = []
    for i, c in enumerate(cards[:6]):
        ...
        for a in (c.get("actions") if isinstance(c.get("actions"), list) else [])[:2]:
            if isinstance(a, dict):
                lines.append(f"  action tag={a.get('tag') or ''!s} text={str(a.get('text') or '')[:70]} href={str(a.get('href') or '')[:100]}")
    return lines
...
def _render_page_ir(ir: dict[str, Any], max_chars: int = 2200) -> str:
    ...
    if forms:
        lines.append("FORMS:")
        lines.extend(_render_ir_forms(forms))
    ...
    if links:
        lines.append("LINKS:")
        lines.extend(_render_ir_links(links))
    cards = ir.get("cards") if isinstance(ir.get("cards"), list) else []
    if cards:
        lines.append("CARDS:")
        lines.extend(_render_ir_cards(cards))
    return "\n".join(lines)[:max_chars]


def _ir_list_to_set(key: str, d: dict[str, Any]) -> set[str]:
    v = d.get(key)
...
def _compute_ir_delta(*, task_id: str, page_ir: dict[str, Any]) -> str:
    ...
    try:
        ...
        prev = st.get("prev_ir") if isinstance(st.get("prev_ir"), dict) else {}
        prev_cta = _ir_list_to_set("ctas", prev)
        cur_cta = _ir_list_to_set("ctas", page_ir)
        prev_heads = _ir_list_to_set("headings", prev)
        cur_heads = _ir_list_to_set("headings", page_ir)
        ...
        st["prev_ir"] = page_ir
        return (
            f"forms:{len(p_forms)}->{len(c_forms)}, cards:{len(p_cards)}->{len(c_cards)}, "
...
def _input_kind_from_blob(blob: str, attrs: dict[str, str]) -> str:
    if "password" in blob or (attrs.get("type") or "").lower() == "password":
        return "password"
    if "email" in blob:
        return "email"
    if any(k in blob for k in ["search", "buscar", "query", "find"]):
        return "search"
    if any(k in blob for k in ["user", "username", "login"]):
        return "username"
    return "text"


def _structured_inputs(candidates: list[_Candidate]) -> list[dict[str, Any]]:
    out = []
    for i, c in enumerate(candidates):
        if c.tag not in {"input", "textarea", "select"}:
            continue
        attrs = {k: (c.attrs or {}).get(k) or "" for k in ("type", "name", "id", "placeholder", "aria-label")}
        ...
        out.append(
            {
                "candidate_id": i,
                "kind": _input_kind_from_blob(blob, attrs),
                "label": label[:80],
                "required": (c.attrs or {}).get("required") is not None,
...
def _structured_clickables(candidates: list[_Candidate]) -> list[dict[str, Any]]:
    clickable = [(i, c) for i, c in enumerate(candidates) if c.tag in {"a", "button"}]
    clickable.sort(key=lambda t: len((t[1].context or "").strip()), reverse=True)
    return [
        {
            "candidate_id": i,
            "tag": c.tag,
...
def _structured_hints(_task: str, candidates: list[_Candidate]) -> dict[str, Any]:
    ...
    return {"inputs": _structured_inputs(candidates), "clickables": _structured_clickables(candidates)}
...
def _llm_json_recover(raw: str) -> dict[str, Any]:
    """Strip code fences and extract first JSON object from raw string."""
    s = raw
    if s.startswith("```"):
        s2 = s[len("```json") :] if s.startswith("```json") else s[len("```") :]
        s = s2[: -len("```")].strip() if s2.endswith("```") else s2.strip()
...
def _llm_is_tool(obj: dict[str, Any]) -> bool:
    t = obj.get("tool")
...
def _task_risk_hint(task: str, step_index: int, candidates: list[_Candidate]) -> str:
    ...
    if step_index <= 1 and n >= 12:
        return (
            "High-risk task detected. Before destructive or irreversible clicks, identify the exact target via find_card/list_cards/search_text using task constraints, then click the matched action."
        )
    return "For high-risk actions, verify target attributes in context first and avoid generic repeated clicks."


class _BrowserStateNode:
    __slots__ = ("children", "items", "name")

    def __init__(self, name: str) -> None:
        self.name = name
        self.children: dict[str, _BrowserStateNode] = {}
        self.items: list[tuple[int, _Candidate]] = []


def _browser_state_chain_for(c: _Candidate) -> list[str]:
    try:
        ch = list(getattr(c, "container_chain", []) or [])
    except Exception:
        ch = []
    if not ch:
        ch = [(getattr(c, "group", "") or "PAGE").strip() or "PAGE"]
...
def _format_browser_state(*, candidates: list[_Candidate], prev_sig_set: set[str] | None) -> str:
    ...
    root = _BrowserStateNode("ROOT")
    for i, c in enumerate(candidates):
        node = root
        for part in _browser_state_chain_for(c):
            if part not in node.children:
                node.children[part] = _BrowserStateNode(part)
            node = node.children[part]
        node.items.append((i, c))
    return "\n".join(_browser_state_render_node(root, prev_sig_set, ""))
...
def _tool_search_text(*, html: str, query: str, regex: bool = False, case_sensitive: bool = False, max_matches: int = 20, context_chars: int = 80) -> dict[str, Any]:
    ...
    try:
        pat = re.compile(q, flags) if regex else re.compile(re.escape(q), flags)
    except Exception as e:
...
def _llm_decide(
    *,
    ...
    for _ in range(max_tool_calls + 2):
        ...
        if _llm_is_tool(obj) and tool_calls < max_tool_calls:
        ...
        if _llm_valid_action(obj, url, candidates):
            with contextlib.suppress(Exception):
                obj["_meta"] = {"llm_calls": len(usages), "llm_usages": usages, "model": str(model), "tool_calls": tool_calls}
            return obj
        ...
        obj = _call(
            f"Your previous JSON was invalid. Fix it. candidate_id must be an integer in [0, {len(candidates) - 1}]. If action is type/select you must include non-empty text. If stuck, scroll_down."
        )
        if _llm_valid_action(obj, url, candidates):
            with contextlib.suppress(Exception):
                obj["_meta"] = {"llm_calls": len(usages), "llm_usages": usages, "model": str(model), "tool_calls": tool_calls}
...
def _act_decision_meta(decision: dict[str, Any]) -> dict[str, Any]:
    meta = decision.get("_meta") or {}
    return {"model": meta.get("model"), "llm": meta}


def _act_handle_navigate(decision: dict[str, Any], effective_url: str, url: str, task_id: str, _resp) -> dict[str, Any]:
    nav_url_raw = str(decision.get("url") or "").strip()
    if not nav_url_raw:
        return _resp([{"type": "WaitAction", "time_seconds": 1.0}], {"decision": "navigate_missing_url"})
    nav_url = _resolve_url(nav_url_raw, effective_url or url)
    ...
    try:
        ...
        pass
    return _resp([{"type": "NavigateAction", "url": nav_url, "go_back": False, "go_forward": False}], {"decision": "navigate", "url": nav_url, **_act_decision_meta(decision)})


def _act_handle_click_href(selector: dict, cid: int | None, href: str, effective_url: str, url: str, task_id: str, decision: dict[str, Any], _resp) -> dict[str, Any]:
    fixed = _preserve_seed_url(href, effective_url or url)
    if not fixed or fixed == href:
    ...
    fixed_abs = _resolve_url(fixed, effective_url or url)
    ...
    try:
        ...
        pass
    return _resp([{"type": "NavigateAction", "url": fixed_abs, "go_back": False, "go_forward": False}], {"decision": "navigate", "url": fixed_abs, "candidate_id": cid, **_act_decision_meta(decision)})


def _act_handle_click(c: _Candidate, cid: int, decision: dict[str, Any], task_id: str, url: str, effective_url: str, _resp) -> dict[str, Any]:
    ...
    try:
        if isinstance(selector, dict) and selector.get("type") == "attributeValueSelector" and selector.get("attribute") == "href":
            ...
            return _act_handle_click_href(selector, cid, href, effective_url, url, task_id, decision, _resp)
    except Exception:
        pass
    _update_task_state(task_id, str(url), f"click:{_selector_repr(selector)}")
    return _resp([{"type": "ClickAction", "selector": selector}], {"decision": "click", "candidate_id": cid, **_act_decision_meta(decision)})
...
def _act_response_for_action(
    ...
    """Dispatch to the right response builder by action type."""
    if action == "navigate":
        return _act_handle_navigate(decision, effective_url, url, task_id, _resp)
    if action in {"scroll_down", "scroll_up"}:
        _update_task_state(task_id, str(url), action)
        cid_int = int(cid) if isinstance(cid, int) else None
        return _resp([{"type": "ScrollAction", "down": action == "scroll_down", "up": action == "scroll_up"}], {"decision": action, "candidate_id": cid_int, **_act_decision_meta(decision)})
    if action == "done":
        _update_task_state(task_id, str(url), "done")
        cid_int = int(cid) if isinstance(cid, int) else None
        return _resp([], {"decision": "done", "candidate_id": cid_int, **_act_decision_meta(decision)})
    ...
    if candidates and step_index < 5:
        ...
        return _resp([{"type": "ClickAction", "selector": selector}], {"decision": "click_override", "candidate_id": 0, **_act_decision_meta(decision)})
    _update_task_state(task_id, str(url), "fallback_wait")
    return _resp([{"type": "WaitAction", "time_seconds": 2.0}], {"decision": "fallback_wait", **_act_decision_meta(decision)})
...
def _state_delta_strings_equal(a: str, b: str) -> bool:
    """Prefix equality when both long (S6659); exact when short."""
    if not a or not b:
        return False
...
class ApifiedWebAgent(IWebAgent):
    ...
    async def act(
        ...
        *,
        ...
        if not callable(create_action_fn):
            logger.error(f"[AGENT_TRACE] BaseAction.create_action missing task_id={task_id} step_index={int(step_index)} BaseAction={BaseAction!r} import_ok={_AUTOPPIA_IWA_IMPORT_OK}")
        payload = {
            "task_id": task_id,
            "prompt": prompt,
        ...
        _log_trace(f"act() raw actions task_id={task_id} step_index={int(step_index)} count={len(actions) if isinstance(actions, list) else 0}")
        out: list[BaseAction] = []
        for a in actions if isinstance(actions, list) else []:
            ...
            try:
                ...
                logger.error(
                    ...
                    f"action_type={a.get('type') or ''!s} err={exc!s} "
                    f"payload={json.dumps(a, ensure_ascii=True)[:500]}"
                )
                continue
        if isinstance(actions, list) and actions and not out:
            logger.error(
                f"[AGENT_TRACE] all actions dropped during conversion task_id={task_id} step_index={int(step_index)} raw_types={[str(x.get('type') or '') for x in actions if isinstance(x, dict)]}"
            )
        return out
    ...
    async def act_from_payload(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(cid, str) and cid.isdigit():
            cid = int(cid)
        out = _act_response_for_action(action, cid, text, decision, candidates, task_id, str(url), effective_url, int(step_index), _resp)
        try:
            action_types = [str(a.get("type") or "") for a in out.get("actions", []) if isinstance(a, dict)]
        except Exception:
            action_types = []
        _log_trace(f"act_from_payload task_id={task_id} step_index={int(step_index)} decision={action!s} candidate_id={cid!s} out_actions={action_types}")
        return out
