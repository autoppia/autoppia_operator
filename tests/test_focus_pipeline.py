...
def test_build_task_cache_override_keeps_prompt_canonical_and_stores_clues_separately(tmp_path: Path) -> None:
    ...
    assert "Navigate directly to /login first." not in tasks[0]["prompt"]
    assert "Navigate directly to /login first." in tasks[0]["prompt_clues"]
