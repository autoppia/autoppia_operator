...
def _search_restaurant_task_row() -> dict:
    return {
        "id": "search-restaurant-sample",
        "web_project_id": "autodining",
        "url": "http://localhost:8003/?seed=499",
        "prompt": "Search for restaurants per task constraints",
        "specifications": {},
        "tests": [
            {
                "type": "CheckEventTest",
...
def test_list_autodining_iwa_use_cases_matches_iwa_loader() -> None:
    from_iwa = frozenset(load_autodining_use_case_completion_flows().keys())
    from_op = list_autodining_iwa_use_cases()
    assert from_iwa == from_op


def test_deterministic_registry_covers_all_iwa_autodining_use_cases() -> None:
    expected = frozenset(load_autodining_use_case_completion_flows().keys())
    registered = {uc for project_id, uc in DETERMINISTIC_PLAN_BUILDERS if project_id == "autodining"}
    assert registered == expected


def test_build_autodining_plan_aligns_first_navigate_to_task_url() -> None:
    row = _search_restaurant_task_row()
    objective = normalize_task_row(row)
    assert isinstance(objective, DeterministicTaskObjective)
    plan = build_autodining_plan(objective)
    assert plan.metadata.get("source") == "iwa_p04_autodining"
    assert plan.actions[0]["type"] == "NavigateAction"
    assert plan.actions[0]["url"] == objective.task_url


def test_build_registered_plan_routes_autodining() -> None:
    row = _search_restaurant_task_row()
    objective = normalize_task_row(row)
