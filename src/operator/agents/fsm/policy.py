        ...
        *,
        ...
        if direct_mode:
            user_parts = [
                ...
                json.dumps(
                    policy_obs.get("active_objective") if isinstance(policy_obs.get("active_objective"), dict) else {},
                    ensure_ascii=False,
                ),
                "",
                "WORKING STATE (JSON):",
                json.dumps(
                    policy_obs.get("working_state") if isinstance(policy_obs.get("working_state"), dict) else {},
                    ensure_ascii=False,
                ),
                "",
                *(
                ...
                "SCORE FEEDBACK (JSON):",
                json.dumps(
                    policy_obs.get("score_feedback") if isinstance(policy_obs.get("score_feedback"), dict) else {},
                    ensure_ascii=False,
                ),
                "",
                "LOCAL WORKFLOW CLOSURE (JSON):",
                json.dumps(
                    policy_obs.get("local_workflow_closure") if isinstance(policy_obs.get("local_workflow_closure"), dict) else {},
                    ensure_ascii=False,
                ),
                "",
                "LOCAL HTML CONTEXT (JSON):",
                json.dumps(
                    policy_obs.get("local_html_context") if isinstance(policy_obs.get("local_html_context"), dict) else {},
                    ensure_ascii=False,
                ),
                "",
                "KNOWN SITE MAP (JSON):",
                json.dumps(
                    policy_obs.get("site_knowledge") if isinstance(policy_obs.get("site_knowledge"), dict) else {},
                    ensure_ascii=False,
                ),
                "",
                "AVOID REPEATING (JSON):",
                json.dumps(
                    policy_obs.get("avoid_repeating") if isinstance(policy_obs.get("avoid_repeating"), dict) else {},
                    ensure_ascii=False,
                ),
                "",
                "PREVIOUS REASONING TRACE (JSON):",
                json.dumps(
                    policy_obs.get("reasoning_trace") if isinstance(policy_obs.get("reasoning_trace"), dict) else {},
                    ensure_ascii=False,
                ),
                "",
                ...
                json.dumps(
                    policy_obs.get("history_recent") if isinstance(policy_obs.get("history_recent"), list) else [],
                    ensure_ascii=False,
                ),
                "",
                "PAGE OBSERVATIONS (JSON):",
                json.dumps(
                    policy_obs.get("page_observations") if isinstance(policy_obs.get("page_observations"), dict) else {},
                    ensure_ascii=False,
                ),
                "",
            ...
            user_parts = [
                ...
                *(
                ...
                json.dumps(
                    policy_obs.get("previous_step_verdict") if isinstance(policy_obs.get("previous_step_verdict"), dict) else {},
                    ensure_ascii=False,
                ),
            ]
            ...
            if recent_failures:
                user_parts.extend(
                    [
                        ...
                        json.dumps(recent_failures[:4], ensure_ascii=False),
                    ]
                )
            ...
            if visual_notes or visual_hints:
                user_parts.extend(
                    [
                        ...
                        json.dumps(
                            {
                                "notes": visual_notes[:8],
                                "element_hints": visual_hints[:12],
                            },
                            ensure_ascii=False,
                        ),
                    ]
            ...
            if meta_enabled:
                user_parts.insert(
                    -5,
                    '{"type":"meta","meta_tool":{"name":"META.FIND_ELEMENTS","arguments":{"role":"input","text":"search","limit":6}}}',
                )
                if "META.VISION_QA" in _obs_meta_tools():
                    user_parts.insert(
                        -5,
                        '{"type":"meta","meta_tool":{"name":"META.VISION_QA","arguments":{"question":"Which visible control best applies the current filters?"}}}',
                    )
        ...
        if t == "browser":
            ...
            for tc in normalized_calls:
                ...
                if name == "browser.done":
                    saw_done = True
                    continue
                if name == "browser.input" and _is_generic_tool_placeholder(
                    _candidate_text(args.get("text"), args.get("value")),
                    kind="input",
                ):
        ...
        try:
            raw = self.llm_call(
                ...
                messages=[
                    ...
                    {
                        "role": "user",
                        "content": json.dumps(repair_user, ensure_ascii=False),
                    },
                ],
                temperature=0.0,
                max_tokens=500,
    ...
    def _fallback(
        ...
        *,
        ...
        if route_like_stuck and allow("browser.go_back"):
            return {
                "type": "browser",
                "tool_call": {"name": "browser.go_back", "arguments": {}},
            }
        ordered = []
        ...
        return {
            "type": "final",
            "done": True,
            "content": "No safe browser action available from allowed_tools.",
        }
