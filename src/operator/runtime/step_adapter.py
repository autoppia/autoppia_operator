...
def run_step_engine(step_engine: Any, payload: dict[str, Any], *, model_override: str) -> dict[str, Any]:
    try:
        ...
        logger.exception(f"[AGENT_TRACE] step_engine_failed task_id={payload.get('task_id') or ''!s} step_index={int(payload.get('step_index') or 0)} err={exc!s}")
        raise HTTPException(status_code=500, detail="step_engine_failed") from exc
