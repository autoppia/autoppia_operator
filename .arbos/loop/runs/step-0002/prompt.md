...
- The low-level selector/execution path improved, but the main remaining issue is policy/workflow drift.
- We now want a dataset-first loop for `autocinema`, not a one-off smoke fix.
- Harvest both successful and failed trajectories for all 16 Autocinema use cases across multiple seeds.
- Prefer reproducible code and saved artifacts over manual shell history.
- Reuse existing trajectory or DAgger tooling if it helps, but keep one stable dataset format.

Known recent observations:

- `LOGIN` and `ADD_TO_WATCHLIST` were still drifting into the wrong workflow early.
- Focus on trustworthy trace persistence, per-use-case summaries, and the highest-leverage workflow fixes.
