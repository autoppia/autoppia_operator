"""Deterministic plans for `autozone` sourced from IWA `p03_autozone/trajectories.py`."""
...
def _trajectory_map() -> dict[str, object]:
    ...
    if _TRAJ_MAP is None:
        loaded = get_trajectory_map("autozone")
        if not loaded:
            raise RuntimeError("IWA returned no trajectory map for autozone; check p03_autozone/trajectories.py")
...
def build_autozone_plan(objective: DeterministicTaskObjective) -> DeterministicPlan:
    if str(objective.web_project_id or "").strip().lower() != "autozone":
        raise ValueError("build_autozone_plan requires web_project_id=autozone")
    ...
    fe = frontend_url_for_project("autozone")
    ...
    prompt_lines = (f"IWA p03 autozone trajectory for {use_case}", f"seed={objective.seed}", prompt)
    return DeterministicPlan(
        prompt_lines=prompt_lines,
        actions=tuple(actions),
        metadata={
            "source": "iwa_p03_autozone",
