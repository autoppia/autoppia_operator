"""Deterministic plans for `autodelivery` sourced from IWA `p07_autodelivery/trajectories.py`."""
...
def _trajectory_map() -> dict[str, object]:
    ...
    if _TRAJ_MAP is None:
        loaded = get_trajectory_map("autodelivery")
        if not loaded:
            raise RuntimeError("IWA returned no trajectory map for autodelivery; check p07_autodelivery/trajectories.py")
...
def build_autodelivery_plan(objective: DeterministicTaskObjective) -> DeterministicPlan:
    if str(objective.web_project_id or "").strip().lower() != "autodelivery":
        raise ValueError("build_autodelivery_plan requires web_project_id=autodelivery")
    ...
    fe = frontend_url_for_project("autodelivery")
    ...
    prompt_lines = (f"IWA p07 autodelivery trajectory for {use_case}", f"seed={objective.seed}", prompt)
    return DeterministicPlan(
        prompt_lines=prompt_lines,
        actions=tuple(actions),
        metadata={
            "source": "iwa_p07_autodelivery",
