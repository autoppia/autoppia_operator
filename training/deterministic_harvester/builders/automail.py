"""Deterministic plans for `automail` sourced from IWA `p06_automail/trajectories.py`."""
...
def _trajectory_map() -> dict[str, object]:
    ...
    if _TRAJ_MAP is None:
        loaded = get_trajectory_map("automail")
        if not loaded:
            raise RuntimeError("IWA returned no trajectory map for automail; check p06_automail/trajectories.py")
        _TRAJ_MAP = dict(loaded)
    return _TRAJ_MAP  # type: ignore[return-value]


def list_automail_iwa_use_cases() -> frozenset[str]:
    return frozenset(_trajectory_map().keys())


def build_automail_plan(objective: DeterministicTaskObjective) -> DeterministicPlan:
    if str(objective.web_project_id or "").strip().lower() != "automail":
        raise ValueError("build_automail_plan requires web_project_id=automail")
    ...
    fe = frontend_url_for_project("automail")
    ...
    prompt_lines = (f"IWA p06 automail trajectory for {use_case}", f"seed={objective.seed}", prompt)
    return DeterministicPlan(
        prompt_lines=prompt_lines,
        actions=tuple(actions),
        metadata={
            "source": "iwa_p06_automail",
