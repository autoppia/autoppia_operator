"""Deterministic plans for `autolist` sourced from IWA `p12_autolist/trajectories.py`."""
...
def _trajectory_map() -> dict[str, object]:
    ...
    if _TRAJ_MAP is None:
        loaded = get_trajectory_map("autolist")
        if not loaded:
            raise RuntimeError("IWA returned no trajectory map for autolist; check p12_autolist/trajectories.py")
        _TRAJ_MAP = dict(loaded)
    return _TRAJ_MAP  # type: ignore[return-value]


def list_autolist_iwa_use_cases() -> frozenset[str]:
    return frozenset(_trajectory_map().keys())


def build_autolist_plan(objective: DeterministicTaskObjective) -> DeterministicPlan:
    if str(objective.web_project_id or "").strip().lower() != "autolist":
        raise ValueError("build_autolist_plan requires web_project_id=autolist")
    ...
    fe = frontend_url_for_project("autolist")
    ...
    prompt_lines = (f"IWA p12 autolist trajectory for {use_case}", f"seed={objective.seed}", prompt)
    return DeterministicPlan(
        prompt_lines=prompt_lines,
        actions=tuple(actions),
        metadata={
            "source": "iwa_p12_autolist",
