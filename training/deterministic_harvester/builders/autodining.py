"""Deterministic plans for `autodining` sourced from IWA `p04_autodining/trajectories.py`."""
...
def _trajectory_map() -> dict[str, object]:
    ...
    if _TRAJ_MAP is None:
        loaded = get_trajectory_map("autodining")
        if not loaded:
            raise RuntimeError("IWA returned no trajectory map for autodining; check p04_autodining/trajectories.py")
...
def build_autodining_plan(objective: DeterministicTaskObjective) -> DeterministicPlan:
    if str(objective.web_project_id or "").strip().lower() != "autodining":
        raise ValueError("build_autodining_plan requires web_project_id=autodining")
    ...
    fe = frontend_url_for_project("autodining")
    ...
    prompt_lines = (f"IWA p04 autodining trajectory for {use_case}", f"seed={objective.seed}", prompt)
