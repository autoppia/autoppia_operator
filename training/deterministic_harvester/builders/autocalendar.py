"""Deterministic plans for `autocalendar` sourced from IWA `p11_autocalendar/trajectories.py`."""
...
def _trajectory_map() -> dict[str, object]:
    ...
    if _TRAJ_MAP is None:
        loaded = get_trajectory_map("autocalendar")
        if not loaded:
            raise RuntimeError("IWA returned no trajectory map for autocalendar; check p11_autocalendar/trajectories.py")
...
def build_autocalendar_plan(objective: DeterministicTaskObjective) -> DeterministicPlan:
    if str(objective.web_project_id or "").strip().lower() != "autocalendar":
        raise ValueError("build_autocalendar_plan requires web_project_id=autocalendar")
    ...
    fe = frontend_url_for_project("autocalendar")
    ...
    prompt_lines = (f"IWA p11 autocalendar trajectory for {use_case}", f"seed={objective.seed}", prompt)
    return DeterministicPlan(
        prompt_lines=prompt_lines,
        actions=tuple(actions),
        metadata={
            "source": "iwa_p11_autocalendar",
