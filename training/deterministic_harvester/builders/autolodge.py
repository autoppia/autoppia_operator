"""Deterministic plans for `autolodge` sourced from IWA `p08_autolodge/trajectories.py`."""
...
def _trajectory_map() -> dict[str, object]:
    ...
    if _TRAJ_MAP is None:
        loaded = get_trajectory_map("autolodge")
        if not loaded:
            raise RuntimeError("IWA returned no trajectory map for autolodge; check p08_autolodge/trajectories.py")
...
def build_autolodge_plan(objective: DeterministicTaskObjective) -> DeterministicPlan:
    if str(objective.web_project_id or "").strip().lower() != "autolodge":
        raise ValueError("build_autolodge_plan requires web_project_id=autolodge")
    ...
    fe = frontend_url_for_project("autolodge")
    ...
    prompt_lines = (f"IWA p08 autolodge trajectory for {use_case}", f"seed={objective.seed}", prompt)
    return DeterministicPlan(
        prompt_lines=prompt_lines,
        actions=tuple(actions),
        metadata={
            "source": "iwa_p08_autolodge",
