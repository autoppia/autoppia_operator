...
1. Provide a task cache file scoped to the project/use case (e.g. `data/task_cache/autocinema_50_tasks.json`, `data/task_cache/autobooks_tasks.json` for web2, `data/task_cache/autozone_tasks.json` for web3, or `data/task_cache/autodining_tasks.json` for web4). Ensure the app for that project is running at the host/port in the task `url` (e.g. autodining defaults to port 8003); otherwise replays can fail with connection errors.
2. Run:
...
- Deterministic planner logic is organized under `training/deterministic_harvester/builders/`, with shared helpers in `common.py`, project-specific logic in `autocinema.py` and IWA-backed `autobooks.py` / `autodining.py` / `autozone.py` (convert IWA `trajectories.py` actions), plus `iwa_planned_actions.py` for IWA action shaping, and a thin registry/facade in `planners.py`.
