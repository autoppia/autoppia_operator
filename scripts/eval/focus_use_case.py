...
def cmd_claude_brief(args: argparse.Namespace) -> int:
    ...
    seed = int(args.seed)
    output_root = focus_root(use_case=use_case)
    payload = generate_claude_brief(web_project_id=str(args.project_id).strip(), use_case=use_case, seed=seed, model=str(args.model).strip() or "claude-sonnet-4-5", task_cache_path=Path(args.task_cache).resolve())
