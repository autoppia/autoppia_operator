...
def _task_replacements_from_cache(*, task_cache_path: Path, use_case: str) -> dict[str, str]:
    try:
    ...
    if not isinstance(rows, list):
        return {}
    target_use_case = str(use_case).strip().upper()
    row = next(
        (
            item
            for item in rows
            if isinstance(item, dict)
            and str(((item.get("use_case") or {}).get("name") if isinstance(item.get("use_case"), dict) else "")).strip().upper()
            == target_use_case
        ),
        None,
    )
    ...
    user_for_login = row.get("relevant_data") if isinstance(row.get("relevant_data"), dict) else {}
    login_payload = user_for_login.get("user_for_login") if isinstance(user_for_login.get("user_for_login"), dict) else {}
    username = str(login_payload.get("username") or "").strip()
    password = str(login_payload.get("password") or "").strip()
    if not username:
        username_match = re.search(r"username\s+'([^']+)'", prompt_text, re.IGNORECASE)
        if username_match:
            username = str(username_match.group(1)).strip()
    if not password:
        password_match = re.search(r"password\s+'([^']+)'", prompt_text, re.IGNORECASE)
        if password_match:
            password = str(password_match.group(1)).strip()
    if username:
        replacements["<web_agent_id>"] = username
        replacements["<username>"] = username
    if password:
        replacements["<password>"] = password
        replacements["password123"] = password
    return {key: value for key, value in replacements.items() if key and value}


def _apply_string_replacements(payload: Any, replacements: dict[str, str]) -> Any:
    if not replacements:
        return payload
    ...
    if isinstance(payload, list):
        return [_apply_string_replacements(item, replacements) for item in payload]
    if isinstance(payload, tuple):
        return tuple(_apply_string_replacements(item, replacements) for item in payload)
    if isinstance(payload, dict):
        return {key: _apply_string_replacements(value, replacements) for key, value in payload.items()}
    return payload
...
def _generate_candidate_attempt(
    *,
    ...
    prompt_override_base = build_prompt_override(use_case=config.use_case, extra_lines=None)
    claude_task_cache_path = _build_task_cache_for_seed(
        config=config,
        seed=seed,
        prompt_override=prompt_override_base,
        suffix=f"_claude_{attempt_idx:02d}_brief",
    )
    brief_payload = generate_claude_brief(
        ...
        task_cache_path=claude_task_cache_path,
    )
    ...
    replacements = _task_replacements_from_cache(task_cache_path=task_cache_path, use_case=config.use_case)
    if replacements:
        candidate = TrajectoryCandidate(
            ...
            prompt_lines=tuple(
                str(item).strip()
                for item in _apply_string_replacements(list(candidate.prompt_lines), replacements)
                if str(item).strip()
            ),
            actions=tuple(
                dict(item)
                for item in _apply_string_replacements([dict(action) for action in candidate.actions], replacements)
                if isinstance(item, dict)
            ),
...
def export_harvest_sft(
    *,
    ...
    from training.format_for_sft import (
