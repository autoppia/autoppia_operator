...
def inject_seed(task: Task, seed: int | None = None) -> tuple[Task, int]:
    ...
    if seed is not None:
        ...
        try:
            query = dict(parse_qsl(urlsplit(str(getattr(t, "url", "") or "")).query, keep_blank_values=True))
            raw_existing = str(query.get("seed") or "").strip()
            if raw_existing:
                existing_seed = int(raw_existing)
        except Exception:
            existing_seed = None
        seed_i = existing_seed if existing_seed is not None else random.randint(1, 100_000)
...
async def run_evaluation(
    ...
    for i, base_task in enumerate(tasks):
        for r in range(reps):
            ...
            # Keep web_agent_id simple and numeric so benchmark credential placeholders
            # like user<web_agent_id> resolve to valid demo-web accounts such as user1.
            episode_web_agent_id = str(episode_index + 1)
